...
logdiff = FunctionalForm(lambda income, price: np.log(income - price), 'log(income - price)')
...
normdiff = FunctionalForm(
    lambda income, price, scale: scipy.stats.norm.cdf(income - price, scale=scale), 'normcdf(income - price)',
    n_params=1, starting_values=np.array([1]), param_names=['normcdf_sd']
    )

# square root of difference, as used in Tra (2010) robustness checks
sqrtdiff = FunctionalForm(lambda income, price: np.sqrt(income - price), 'sqrt(income - price)')
