...
def vehchoice ():
    ...
    return pd.read_parquet(os.path.join(os.path.dirname(__file__), 'data', 'veh_choice.parquet.gz'))
...
def vehchoice_alts ():
    ...
    est_data = pd.read_parquet(os.path.join(os.path.dirname(__file__), 'data', 'veh_choice.parquet.gz'))
    ...
    alts = pd.concat([
        get_data_for_choice(cars, sfh) for cars, sfh in product([0, 1, 2, 3], [0, 1])
    ], ignore_index=True).fillna(0).sort_values(['houseid', 'alt_sfh', 'alt_cars']).reset_index(drop=True)
    ...
    alt_data = alts.drop(columns=['houseid', 'alt_cars', 'alt_sfh', 'sfh_act', 'cars_act'])
...
def test_hessian (vehchoice, vehchoice_alts, monkeypatch):
    ...
    mfa = MNLFullASC(
        vehchoice_alts['data'],
        supply=(
            np.bincount(vehchoice.nveh),
            np.bincount(vehchoice.sfh)
        ),
    ...
    hess_sm = statsmodels.tools.numdiff.approx_hess3(mfa.params.to_numpy(), mfa.negative_log_likelihood)
