...
def test_compute_ascs():
    ...
    choiceidx = [np.tile(np.repeat(np.arange(4), 4), 4), np.tile(np.arange(4), 16)]
    ...
    ascs = compute_ascs(
        base_utilities=utilities, supply=supply, hhidx=hhidx, choiceidx=choiceidx
    )

    # compute probabilities
    full_exp_utilities = np.exp(
        utilities + ascs[0][choiceidx[0]] + ascs[1][choiceidx[1]]
    )
    ...
    assert np.allclose(
        shares_0, supply[0]
    ), "ascs do not produce correct market shares for dim 0"
    assert np.allclose(
        shares_1, supply[1]
    ), "ascs do not produce correct market shares for dim 1"
